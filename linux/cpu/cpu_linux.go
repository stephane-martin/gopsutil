...
func (c *CPU) Times(percpu bool) ([]TimesStat, error) {
	return c.TimesWithContext(context.Background(), percpu)
}

func (c *CPU) TimesWithContext(ctx context.Context, percpu bool) ([]TimesStat, error) {
	filename := common.HostProc("stat")
	var lines = []string{}
	if percpu {
		statlines, err := common.RemoteReadLines(c.sftpClient, filename)
		...
		lines, _ = common.RemoteReadLinesOffsetN(c.sftpClient, filename, 0, 1)
	}

	ret := make([]TimesStat, 0, len(lines))

	for _, line := range lines {
		ct, err := parseStatLine(line, c.tick)
		if err != nil {
			continue
		}
		ret = append(ret, *ct)
...
func finishCPUInfo(client *sftp.Client, c *InfoStat) error {
	var lines []string
	var err error
	var value float64

	if len(c.CoreID) == 0 {
		lines, err = common.RemoteReadLines(client, sysCPUPath(c.CPU, "topology/core_id"))
		if err == nil {
			c.CoreID = lines[0]
		}
	}
	...
	lines, err = common.RemoteReadLines(client, sysCPUPath(c.CPU, "cpufreq/cpuinfo_max_freq"))
...
func parseStatLine(line string, tick float64) (*TimesStat, error) {
